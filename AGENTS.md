...
**Keep this block, including in user-authorized commits that otherwise modify `AGENTS.md`.** It is part of the project's agent setup, maintained by `next dev` for every agent that works here. If it appears as an uncommitted change, preserve it. Do not remove it to clean up a diff; it will be regenerated.
