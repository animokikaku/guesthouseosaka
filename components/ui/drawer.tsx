...
function DrawerContent({ className, ...props }: DrawerPrimitive.Content.Props) {
  return (
    <DrawerPrimitive.Content
      ...
      className={(state) =>
        cn(
          "flex min-h-0 flex-1 flex-col outline-none transition-opacity duration-300 ease-[cubic-bezier(0.45,1.005,0,1.005)] group-data-nested-drawer-open/popup:opacity-0 group-data-nested-drawer-swiping/popup:opacity-100",
...
export const drawerPopupClassName = ({
  ...
  return cn(
    "group/popup relative flex flex-col overflow-hidden",
    "[--bleed:3rem] border-border bg-popover text-popover-foreground overscroll-contain data-swiping:select-none",
...
function DrawerHandle({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "mx-auto mt-4 h-1 w-[100px] shrink-0 rounded-full bg-muted",
        className,
      )}
      {...props}
    />
